...
染色球可用于<ItemLink id="color_applicator" />中，为可染色的方块上色，例如[线缆](cables.md)、羊毛、陶瓦、玻璃和混凝土。它们也可以用于<ItemLink id="matter_cannon" />中，射击出一小片颜料。
...
它们的作用与普通染色球完全相同，但从<ItemLink id="matter_cannon" />射出的颜料斑点会发出光亮。某种意义上可以当作信号枪来用。
